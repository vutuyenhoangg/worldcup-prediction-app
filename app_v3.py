...
def check_base_database():
    if not os.path.exists(DB_PATH):
        st.error(
            ...
            "Bạn cần đặt file `worldcup_prediction.db` cùng cấp với `app.py`."
        )
        st.stop()
    ...
    if "matches" not in table_names:
        st.error("Database chưa có bảng `matches`. Hãy kiểm tra lại bước tạo data.")
        st.stop()
        ...
        if existing is None:
            ...
            cursor.execute(
                ...
                (
                    ...
                    existing["predicted_winner_team_id"],
                    predicted_home_score,
                    predicted_away_score,
                    predicted_winner_team_id,
                    now_text
